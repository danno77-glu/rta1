...
const AuditView = () => {
  ...
  const [expandedPhoto, setExpandedPhoto] = useState(null);
    const [editorHtml, setEditorHtml] = useState(''); // State for Quill editor content
    const quillRef = useRef(null);

    useEffect(() => {
        if (audit) {
            setEditorHtml(audit.notes || ''); // Initialize with audit notes
        }
    }, [audit]);

    const handleQuillChange = (html) => {
        setEditorHtml(html);
        // Directly update the editedAudit state.
        handleAuditChange({ target: { name: 'notes', value: html } });
    };
            ...
            <div className="form-field full-width">
              <label>Notes</label>
                {isEditing ? (
                    ...
                    <div className="field-value" dangerouslySetInnerHTML={{ __html: audit.notes || '' }} />
                )}
            </div>
          </div>
